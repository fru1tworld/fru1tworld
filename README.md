# Introduction 
안녕하세요. 
최근에 [velog](https://velog.io/@fru1tworld/posts)를 하고 있어요.

## 기술스택

### 익숙해요

TypeScript, Spring, MySQL, PostgreSQL, Redis, Elasticsearch, Apache Kafka, Git, Linux, Docker, Vim, AWS

### 할 줄 알아요

Python, TypeScript, Java, Kubernetes

### 관심 있어요

Kotlin, Rust, Go, ML/DL



## Problem Solving
### BOJ 
[![Solved.ac Profile](http://mazassumnida.wtf/api/v2/generate_badge?boj=ilovecoffee)](https://solved.ac/ilovecoffee/)

### CodeForces
[![Codeforces Stats](https://codeforces-readme-stats.vercel.app/api/card?username=fru1t)](https://codeforces.com/profile/fru1t)

### Leetcode
![Leetcode Stats](https://leetcard.jacoblin.cool/fru1t_)


# 레포 소개
## 요즘하고 있어요
[code kata](https://github.com/fru1tworld/code-kata)
- 다양한 언어로 코딩 연습을 하고 있어요.

[직접 만들면서 배우는 가상 면접 사례로 배우는 대규모 시스템 설계](https://github.com/fru1tworld/system-design-interview-practice)
- Kotlin, Ktor, Java, Spring 활용해서 책에서 배운 내용을 직접 구현해요.
[cs-study](https://github.com/fru1tworld/cs-study)
- 개인 CS 학습 레포에요. 각종 CS 책에 대한 개인적인 추천, 난이도가 포함되어있고, 최근에는 다양한 자료를 모아서 읽고 있어요.

[직접 만들면서 배우는 LLM](https://github.com/bllm-study/cohebot)
- LLM을 만들고 있어요.

## 예전에 했어요.
[fru1t OS](https://github.com/fru1tworld/fru1t-os)
- 운영체제를 만들어봤어요. 
[fru1t board](https://github.com/fru1tworld/fru1tboard)
- 백엔드 연습을 위한 게시판 프로젝트
[honeyflow](https://github.com/boostcampwm-2024/web29-honeyflow)
- honeyflow WebSocket, CRDT 기술을 활용한 동시 편집 협업 플랫폼
[cs-basic](https://github.com/fru1tworld/cs-basic)
- cs-study에 있던 면접 질문지를 옮기고, 다양하게 공부할 수 있는 자료를 AI를 활용해서 첨부했어요.
-[tp-cli](https://github.com/fru1tworld/tp-cli)
- 다양한 Shell 환경에서 디렉터리 이동을 편하게 도와주는 CLI에요.
